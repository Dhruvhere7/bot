...
TOKEN = ""  # <- apna token daal
ALLOWED_OWNERS = [871952588394877048, 1289995114231631925, 1402661386294919169]  # multiple owners allowed
...
class ManageView(discord.ui.View):
    ...
    async def update_embed(self, interaction: discord.Interaction, msg: str = None):
        ...
        embed.set_footer(text="🚀 Powered by Fluid Nodes")
    ...
    async def delete_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id not in ALLOWED_OWNERS and interaction.user.id != self.owner_id:
...
async def create_vps(interaction: discord.Interaction, name: str, password: str, owner: discord.Member):
    ...
    # FIX: set password with env variable at creation time
    code, _, err = await run_cmd(
        "docker", "run", "-d",
